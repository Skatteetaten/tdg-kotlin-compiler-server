...
class CompletionProvider(
    ...
    fun complete(
        ...
        elementAt(line, character)?.let { element ->
            ...
            descriptorInfo.descriptors
                ...
                .filter { descriptor -> harTdssInputAnnotasjon(descriptor.annotations) }
                .toMutableList().apply {
                    sortWith { a, b ->
    ...
    private fun harTdssInputAnnotasjon(annotasjoner: Annotations): Boolean {
        return annotasjoner.findAnnotation(FqName("no.skatteetaten.rst.tdss.TdssInput")) != null
    }
    ...
    private fun completionVariantFor(
        ...
        return if (prefix.isEmpty() || fullName.startsWith(prefix)) {
            Completion(
                text = completionText,
                displayText = fullName.replace("\\(.*\\)".toRegex(), ""),
                tail = formaterTail(type, descriptor),
                icon = iconFrom(descriptor)
            )
        } else null
    }

    private fun formaterTail(tail: String, descriptor: DeclarationDescriptor): String {
        val tailUtenNullableChar = tail.replace("\\?".toRegex(), "")

        if (tailUtenNullableChar.contains("Unit")) {
            // Hvis descriptor er av typen Property, bytt ut Unit med "Type"
            if (descriptor is PropertyDescriptor) {
                return tailUtenNullableChar.replace("Unit".toRegex(), "Type")
            }

            // Ellers bytt ut med "Funksjon"
            return tailUtenNullableChar.replace("Unit".toRegex(), "Funksjon")
        }

        return tailUtenNullableChar
    }
